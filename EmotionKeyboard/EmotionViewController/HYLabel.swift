...
public class HYLabel: UILabel {
    ...
    fileprivate typealias HYTapHandler = (HYLabel, String, NSRange) -> Void
    fileprivate var linkTapHandler : HYTapHandler?
    fileprivate var topicTapHandler : HYTapHandler?
    fileprivate var userTapHandler : HYTapHandler?
    ...
    fileprivate func prepareText() {
        ...
        if let linkRanges = getLinkRanges() {
            ...
            for range in linkRanges {
                textStorage.addAttribute(NSForegroundColorAttributeName, value: UIColor.blue, range: range)
            }
        }
